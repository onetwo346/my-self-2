...
const voiceSpeedSelect = document.getElementById('voice-speed-select'); // New: Add <select> in HTML
...
let voiceSpeed = localStorage.getItem('voiceSpeed') || 1.0; // New: Manual speed
...
let conversationMemory = JSON.parse(localStorage.getItem('conversationMemory')) || {
    ...
    trainingSets: [] // New: Saved convos for training
};
...
function parseChatCommand(text) {
    ...
    if (/set voice speed\s*(\d*\.?\d+)/i.test(text)) {
        ...
        if (speed >= 0.5 && speed <= 2.0) {
            ...
            voiceSpeedSelect.value = voiceSpeed; // Sync dropdown
...
function saveAsTraining() {
    ...
    displayChatHistory(); // Refresh sidebar
}
...
function addMessage(text, isUser = false) {
    ...
    if (!isUser && 'responsiveVoice' in window && !isVoiceMuted) {
        responsiveVoice.speak(text, currentVoice, { rate: parseFloat(voiceSpeed) }); // Manual speed
    }
}
