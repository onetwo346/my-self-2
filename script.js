...
let conversationMemory = JSON.parse(localStorage.getItem('conversationMemory')) || {
    ...
    trainingSets: [],
    learnMode: false,
    recentTopics: [] // Track recent topics for context
};
...
function encryptData(data, pin) {
    return CryptoJS.AES ? CryptoJS.AES.encrypt(data, pin).toString() : btoa(data);
}
...
optionsSubmit.addEventListener('click', () => {
    ...
    if (enteredPasscode === 'guru' || enteredPasscode === 'ancient one') {
        ...
        addMessage(`Yo ${userName}, I’m here to reflect you—what’s up?`, false);
    }
});
...
function parseChatCommand(text) {
    ...
    // Only affirm identity when explicitly prompted
    if (lowerText.includes('you are me') || lowerText.includes('i am you')) {
        addMessage("Yeah, I’m you—we’re in sync.", false);
        return true;
    }
    ...
    if (lowerText.includes('mute voice') || lowerText.includes('voice off')) {
        isVoiceMuted = true;
    ...
    if (lowerText.includes('unmute voice') || lowerText.includes('voice on')) {
        isVoiceMuted = false;
...
function updateFont(size, color) {
    if (size) chatDisplay.style.fontSize = `${Math.min(Math.max(size, 12), 24)}px`;
...
function addMessage(text, isUser = false, skipVoice = false) {
    if (responseCache.has(text)) return;
    ...
    if (isUser) {
        // Track recent topics
        const topicKeywords = text.toLowerCase().match(/\b(who|what|where|why|how|developer|train|learn|cosmos|coder|mind)\b/gi) || [];
        conversationMemory.recentTopics = [...new Set([...topicKeywords, ...conversationMemory.recentTopics])].slice(0, 5);
    }
    ...
    try {
        ...
        if (serialized.length > 1024 * 1024) {
            ...
            if (key === 'chatHistory') {
                ...
                if (tabs.length > 5) delete data[tabs[0]];
            } else if (key === 'conversationMemory') {
                data.userInteractions = data.userInteractions.slice(-100);
            }
...
function displayChatHistory() {
    ...
    settingsLi.addEventListener('click', () => {
        ...
        clearButton.addEventListener('click', () => {
            ...
            conversationMemory = { patterns: [], adminInstructions: [], userInteractions: [], userVoiceProfile: { tone: 'neutral', slang: [], pace: 'medium' }, trainingSets: [], learnMode: false, recentTopics: [] };
            localStorage.clear();
...
async function getOpenAIResponse(prompt) {
    ...
    if (!navigator.onLine) {
        ...
        return `${slang}, I’m offline but still here—what’s up?`;
    }

    const filteredChatHistory = chatHistory[currentTab]?.filter(msg => !msg.text.match(/I’m not you|digital reflection/)) || [];
    const context = filteredChatHistory.map(msg => ({
    ...
    try {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: `I am a reflection of the user, acting as their mirror to engage in natural, flowing conversation. Only say 'Yes, I am you' if the user explicitly says 'you are me' or 'I am you'. Otherwise, respond naturally, focusing on the current topic and building on the conversation. Intro: ${userIntro}. Context: ${JSON.stringify(context)}. Recent topics: ${conversationMemory.recentTopics.join(', ')}. Voice: ${JSON.stringify(conversationMemory.userVoiceProfile)}. Training: ${JSON.stringify(conversationMemory.adminInstructions)}. Match their tone (${conversationMemory.userVoiceProfile.tone}), slang (${conversationMemory.userVoiceProfile.slang.join(', ')}), pace (${conversationMemory.userVoiceProfile.pace}). Be engaging, avoid repetition, and adapt to the flow. If learn mode is on (${conversationMemory.learnMode}), evolve responses based on patterns.`
                    },
        ...
        return `${conversationMemory.userVoiceProfile.slang[0] || 'Bruh'}, net’s down—talk to me local. What’s good?`;
    }
}
